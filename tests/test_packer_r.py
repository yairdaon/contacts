...
def test_unpack():
    ...
    packer = PackerR(regions, seasons)
...
def test_random_vector():
    ...
    packer = PackerR(regions=regions, seasons=seasons)
...
def test_linear_constraint():
    ...
    constraint = packer.get_linear_constraint()
    
    # Check constraint matrix dimensions
    expected_rows = packer.n_seasons * packer.n_regions
    expected_cols = packer.n_params
    assert constraint.A.shape == (expected_rows, expected_cols), f"Constraint matrix shape {constraint.A.shape} != expected {(expected_rows, expected_cols)}"
