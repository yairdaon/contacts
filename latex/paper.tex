...
by initial-condition fitting and by a shared national driver. Across
47 states and 12 seasons of US pneumonia and influenza mortality, the
coupled SIR appears to reject the null of no coupling, in apparent
tension with our theoretical prediction. A model-free conditional
Granger test on SARIMA forecasts locates the confounder: another
state's lag-one mortality carries no information beyond an
national average lag. Extending the SIR force of infection to include
a national driver recovers the theoretical prediction: the coupling
parameter collapses toward zero and the likelihood ratio no longer
rejects for the majority of state pairs. Two independent
methodological frameworks converge on the same conclusion: pairwise
coupling for seasonal influenza is redundant given a shared national
driver and should be dropped from forecast models in accordance with
Occam's razor.
\end{abstract}
...
include coupling, only a national average aggregate.
...
details) rejects the null $H_0: \theta = 0$ for $PLACEHOLDER$ of state
...
\caption{Scatter plot $\hat{\theta}$ (x-axis) and standard deviation
  (y-axis) across the 1224 US state pairs. The rejection threshold is
  dotted black. Above the rejection threshold $H_0:\theta=0$ is not
  rejected; below the rejection threshold $H_0:\theta=0$ is rejected in
  favor of the alternative $H_1:\theta > 0$ (see Section
  \ref{subsec:hyp}). The bulk of pairs sits to below the rejection
  line ($PLACEHOLDER$ reject $H_0: \theta = 0$) in accordance with
  \cite{thivierge2025}.}
\label{fig:theta vs crlb coupled}
\end{figure}

\subsubsection{Entrainment alone can make coupling unidentifiable}\label{subsec:crlb_inflated}
Simulations hint that entrainment is key to understanding the behavior
observed in the previous section. We ask whether the coupling
parameter $\theta$ is even in principle recoverable from observed
incidence generated via a two-region SIR model. In
...
$\varepsilon$, comparing in-phase entrainment ($\varphi_1 =
...
enough information about $\theta$ that makes the coupling parameter
...
detectability of pairwise coupling, see the Discussion for a
theoretical explanation.

\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth]{../pix/power.pdf}
...
fit. But if directional coupling is not the underlying cause for
...
Fig.~\ref{fig:lrt}: (a) detectable coupling, in which one state's past
actually affects the other state's future beyond its past and shared
seasonality; or (b) a shared national forcing, in which one state's
...
under (a) we expect a reduction in in-sample RMSE going from
\textbf{national driver} to \textbf{both}, while under (b) we expect
no such reduction since the national average carries all the
information pertaining to the severity. Hence, a comparison of RMSEs
between \textbf{national average} and \textbf{both} will allow us to
discern (a) from (b).


\subsection{SIR model: \textbf{national average} vs.~\textbf{both}}\label{subsec:natavg_results}
...
\textbf{national driver} for $PLACEHOLDER$ of state pairs in favor of
\textbf{both} (compare to $PLACEHOLDER$ of rejecting
...
\caption{Scatter plot $\hat{\theta}$ (x-axis) and standard deviation
  ...
  threshold ($PLACEHOLDER$ reject $H_0: \theta = 0$). Compare to
  PLACEHOLDER of Fig.~\ref{fig:theta vs crlb coupled}.}
\label{fig:theta vs crlb both}
\end{figure}


\subsubsection{SARIMA: \textbf{national average} vs.~\textbf{both}}\label{subsec:sarima_alpha}
We now revisit the model-free autoregressive framework of
Fig.~\ref{fig:sarima_heatmap}. Again, we compare a null
\textbf{national average} to the alternative \textbf{both}. We note
...
\caption{Distribution of relative test-period RMSE change
  ...
  includes the lag-one national average aggregate $\bar y_{t-1}$
...
severity, which the national average aggregate captures cleanly. Had coupling
...
national average-only model. We see no such effect. Conditional Granger
causality then implies the data are consistent with a common-cause
graphical structure $Y_B \leftarrow \bar Y \to Y_A$, in which
national-level severity confounds the apparent directed-coupling
structure $Y_B \to Y_A$.


\section{Discussion}
Our complementary analyses --- a mechanistic SIR fit and, a model-free
conditional Granger test --- arrive at an identical conculsion:
...
inter-state coupling is structurally unidentifiable from observed
...
We believe the Fisher information holds the key to understanding this
behaviour. Partition the $6\times 6$ Fisher information matrix into a
a $5\times 5$ nuisance-parameter block and a $1\times 1$
$\theta$-block,
\begin{equation*}
...
weight. By the Schur complement,
...
no effect on trajectories. Alternatively, (ii) the
$\theta$-sensitivity $\partial\mu/\partial\theta$ is close to the span
of the nuisance-parameter sensitivities $\{\partial\mu/\partial
S_i(0), \partial\mu/\partial I_i(0), \partial\mu/\partial\gamma\}$
(see SI Section~\ref{sec:si_schur} for details). We expect the
...
%% proxy for national-level severity, not a signature of directional
%% inter-state coupling. Extending the mechanistic FOI with a national
%% driver $\gamma$ dissolves the apparent coupling signal from within
%% the SIR framework: once the shared driver is explicit in the model,
%% $\hat\theta$ collapses toward zero and the likelihood ratio
%% $\Lambda_B$ falls below the rejection threshold for most state pairs,
%% recovering the theoretical prediction. In this sense $\theta$ is not
%% so much identifiable as it is a receptacle for shared driving that
...
states contribute essentially nothing beyond the national average
aggregate. Our findings therefore complement those of
\citet{thivierge2025} at the mortality endpoint: the other-state gains
they observed are explained by shared national severity rather than
inter-state coupling, and our $\gamma$-extended SIR fit provides the
mechanistic reason --- coupling and a shared driver are exchangeable
in fitting the observed trajectory, and only one of them is required.
...
subtraction. Sensitivity analyses at $k \in \{5, 20, 50\}$ (SI) show
that our qualitative conclusions are robust to this choice.
...
Table~\ref{tab:cases}, mirroring the SIR configurations of
Section~\ref{subsec:model}.
...
%
%
where $t$ is measured in years. We let $I_{\text{nat}}$ the number of
national infecteds, and $N_{\text{nat}}$ the national population. The
...
We would like to calculate the Fisher information for $I(\theta)$ for
two reasons. First, under standard regularity conditions, the
...
gives rise to the Cramer-Rao lower bound which bounds the variance of
any unbiased estimator from below
\begin{equation}\label{eq:crlb}
  \text{Var}(\hat{\theta}) \geq I(\theta)^{-1}.%[\mathbf{J}^{-1}]_{\theta\theta}.
\end{equation}

We let $\mathbf{J}$ the $6 \times 6$ Fisher Information Matrix with
entries $J_{kl} =
...
$\mathbf{W}^{1/2}\mathbf{G}$. In the Discussion we will take advantage
of this matrix to gain intuition on the structure of the problem at
hand.
...
\end{equation}
A the likelihood ratio test using $\Lambda = 2\bigl[\ell(H_1) -
  \ell(H_0)\bigr]$ gives an asymptotically equivalent test.


\subsection{Simulated Analysis}\label{subsec:sim_crlb}
We simulate eidemics across a grid of coupling values $\theta \in
...
wrote the paper.  This research was supported by The Israel Science
Foundation (grant No. 2332/26).
