...
complement. Partition the $6\times6$ Fisher information matrix into a
$1\times1$ $\theta$-block and a $5\times5$ nuisance-parameter block,
\begin{equation*}
  ...
  \end{bmatrix},
\end{equation*}
where $\eta = (S_1(0), I_1(0), S_2(0), I_2(0), \alpha)$ collects the
four per-region initial conditions and the national-driver mixing weight. By the Schur complement
\begin{equation}\label{eq:schur}
  [\mathbf{J}^{-1}]_{\theta\theta}
  \;=\; \bigl( J_{\theta\theta}
...
the nuisance-parameter sensitivities (see SI
Section~\ref{sec:si_schur}). This makes sense: if $\partial \mu /
\partial \theta$ is in the span of $\{\partial \mu / \partial S_i(0),
\partial \mu / \partial I_i(0), \partial \mu / \partial \alpha\}$,
then any effect the coupling $\theta$ has on disease trajectory is
absorbed by the initial conditions and the national driver. Both
cases lead to the same conclusion: under~(i) coupling by itself is
redundant; under~(ii) the effect of coupling is absorbed by fitting
initial conditions and/or the national-average driver --- something
many forecast routines already do. A large CRLB therefore implies
that pairwise coupling should not improve forecasts.
...
\lambda_i(t) = \beta_i(t)\left[(1-\alpha)\,\frac{\sum_j C_{ij}\, I_j(t)}{\sum_j C_{ij}\, N_j}
\;+\; \alpha\, \frac{I_{\text{nat}}(t)}{N_{\text{nat}}}\right],
\end{equation}
where $\alpha \in [0, 1]$ mixes the local, contact-weighted term with
an exogenous national driver $I_{\text{nat}}(t)/N_{\text{nat}}$. The
national driver is estimated from aggregate US P\&I excess mortality
as $I_{\text{nat}}(t) = Y_{\text{nat}}(t)/\rho$, where
$Y_{\text{nat}}(t)$ is total weekly excess mortality summed across all
included states, and $N_{\text{nat}}$ is the corresponding total
population. When $\alpha = 0$ the model reduces to the standard
two-region metapopulation with pairwise coupling
parameter~$\theta$; when $\alpha > 0$ each region also experiences a
component of infection pressure proportional to national incidence,
independent of pairwise connectivity. This decomposition is the
mechanistic analogue of the SARIMA conditioning on national
mortality: it lets us test, within the SIR framework, whether
pairwise coupling adds anything after a shared national driver is
accounted for.
...
%
%
We fix $k = 10$ throughout and verify robustness to this choice in the
Supplementary Information.

The unknown parameters for a single influenza season are
$\boldsymbol{\phi} = (S_1(0), I_1(0), S_2(0), I_2(0), \alpha, \theta)$, giving
the log-likelihood:
...
with respect to parameters. Let $\phi \in \{S_1(0), I_1(0), S_2(0),
I_2(0), \alpha, \theta\}$ denote a parameter ($\phi = \phi_k$ for $k = 1,
\dots, 6$). It is convenient to write the FOI~\eqref{eq:foi} as
$\lambda_i = \beta_i(t)\bigl[(1-\alpha)\,L_i + \alpha\,\tilde{N}_i\bigr]$
where $L_i(t) = (\mathbf{C}\mathbf{I})_i/(\mathbf{C}\mathbf{N})_i$ is
the local, contact-weighted per-capita infected fraction and
$\tilde{N}_i(t) = I_{\text{nat}}(t)/N_{\text{nat}}$ is the exogenous
national driver (independent of $S_i, I_i$ and of $\alpha, \theta$).
For any $\phi \in \{S_1(0), I_1(0), S_2(0), I_2(0), \theta\}$,
\begin{equation}\label{eq:sens_foi}
\frac{\partial \lambda_i}{\partial \phi} = \beta_i(t)\,(1-\alpha)\left[
...
\right];
\end{equation}
for $\phi = \alpha$, the contact matrix has no $\alpha$-dependence and
$\tilde{N}_i$ contributes an additive term,
\begin{equation}\label{eq:sens_foi_alpha}
\frac{\partial \lambda_i}{\partial \alpha} = \beta_i(t)\left[
\tilde{N}_i - L_i
+ \frac{(1-\alpha)}{(\mathbf{C}\mathbf{N})_i}
\left(\mathbf{C}\,\frac{\partial \mathbf{I}}{\partial \alpha}\right)_i
\right].
\end{equation}
...
where $\mathbf{J}$ is the $6 \times 6$ Fisher Information Matrix with
entries $J_{kl} =
\mathbb{E}[(\partial\ell/\partial\phi_k)(\partial\ell/\partial\phi_l)]$,
indexed by $\boldsymbol{\phi} = (S_1(0), I_1(0), S_2(0), I_2(0),
\alpha, \theta)$.
Let $r_i(t) := Y_i(t) - \rho\mu_i(t)$ denote the residual and
%
%
\begin{equation*}
...
\begin{align}\label{eq:fim}
  ...
  \mathbf{G} &= \begin{pmatrix}
    ...
    \dfrac{\partial \mu_2(T{-}1)}{\partial \alpha} &
...
\text{Var}(\hat{\theta}) \geq \left(\sum_{s} \frac{1}{
  [\mathbf{J}^{-1}(s)]_{\theta\theta}}\right)^{-1} =: \text{CRLB},
\end{equation}
...
no information and are assigned $[\mathbf{J}^{-1}(s)]_{\theta\theta} = \infty$.
...
%
$$
\ell_{\text{PL}}(\theta) = \max_{S_1(0), I_1(0), S_2(0), I_2(0), \alpha}
\ell(S_1(0), I_1(0), S_2(0), I_2(0), \alpha, \theta).
$$
%
%
