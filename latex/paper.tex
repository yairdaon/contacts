...
\title{Cram\'er-Rao and Granger causality show that inter-state coupling adds no information to US influenza forecasts}
...
\begin{abstract}
Influenza forecasts in the US typically encode inter-state
connectivity. Although forecasts appear to improve by including other
states' lagged incidence in a forecast model, the source of this
improvement is unclear. We apply the Cram\'er-Rao Lower Bound to a
two-state SIR model and show that either (i) connectivity does not
affect disease trajectory, or (ii) its effect is absorbed by initial
conditions. Thus, for mechanistic forecasts that fit initial
conditions, connectivity is redundant. Fitting the SIR model to US
pneumonia and influenza mortality confirms that data are consistent
with vanishing connectivity. Independently, a model-free conditional
Granger test shows that another state's mortality provides no
improvement in forecast skill beyond the national average. Both lines
of analysis agree: inter-state connectivity does not improve US
seasonal influenza forecasts and may be omitted from preparedness
models, in accordance with Occam's razor.
\end{abstract}

\noindent\textbf{Keywords:} Fisher information $|$ identifiability $|$
conditional Granger causality $|$ seasonal influenza $|$ entrainment
...
independent oscillator is driven by a periodic forcing; for influenza,
the forcing is environmental \cite{shaman2009, shaman2010}. Although
entrainment can be indistinguishable from mutual synchronization, the
former requires only a unidirectional forcing, while the latter
requires bidirectional coupling, such as population mixing in the case
of influenza. In the United States, all states share
...
connectivity between regions is fundamentally unidentifiable when
...
fitting initial conditions --- something many forecast routines do. A
large CRLB therefore implies that connectivity should not improve
forecasts.
...
seasonal influenza forecasts. We reach this conclusion via two
