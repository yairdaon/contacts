...
\begin{abstract}
Distinguishing entrainment by a shared exogenous drive from genuine
synchronization through inter-region coupling is a recurring problem
in the nonlinear dynamics of living systems, from coupled genetic
oscillators to the spread of infectious diseases. We argue that for
seasonal epidemics the two mechanisms are typically not
distinguishable from population-level observations alone. We apply the
Schur complement to the Fisher information matrix of a coupled SIR
...
shared national driver, rendering the coupling parameter practically
unidentifiable. We then analyze 12 seasons of pneumonia and influenza
mortality records from 47 US states. Somewhat surprisinglu, a
model-free SARIMA fit shows improved prediction skill for 65\% of
state pairs, while a mechanistic coupled SIR fit rejects a null of no
coupling for 80\% of state pairs. However, once a national driver
covariate is included in the analysis, these figures drop to
50\% and 20\%, respectively. We conclude that inter-region coupling
plays no detectable role for seasonally driven influenza and make a
practical recommendation: pairwise coupling should be dropped from
operational influenza forecast models in accordance with Occam's
razor.  More broadly, we recommend the same Fisher-information
diagnostic as an identifiability check for any entrained system where
coupling is suspected.
\end{abstract}
...
The overall behavior of any entrained coupled system could be
understood via the Schur complement of the Fisher information
...
nuisance-parameter block and a $1\times 1$ couplinge block,
\begin{equation*}
  \mathbf{J} =
  %
  %
...
\textbf{both} (compare to $80.7\%$ of rejecting
...
\caption{Scatter plot $\hat{\theta}$ (x-axis) and standard deviation
  ...
  $80.7\%$ of Fig.~\ref{fig:theta vs crlb coupled}.}
...
The authors used Anthropic's Claude (Claude Code, Opus-4.7) to assist
