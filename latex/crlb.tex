...
\end{equation}
where $\varepsilon_i(t) \sim \mathcal{N}(0, \sigma^2)$ independently, for $t = 0, 1, \ldots, T$.
...
\vdots \\
\beta(T) S_1(T)[I_1(T) + \theta I_2(T)] \\
\beta(T) S_2(T)[\theta I_1(T) + I_2(T)]
\end{bmatrix}
...
derivatives are:
\begin{align}
\frac{\partial \mu(t)}{\partial \theta} &= \beta(t) S(t) \circ \Omega I(t) \\
\frac{\partial \mu(t)}{\partial y} &= \beta(t)\left [ \frac{\partial S(t)}{\partial y} \circ C I(t) + S(t) \circ C \frac{\partial I(t)}{\partial y}\right ] 
\end{align}
...
&= \sigma^{-2} \sum_{t=0}^T \beta(t)^2\left[ S_1^2(t) I_2^2(t) + S_2^2(t) I_1^2(t) \right]
\end{align}
...
\begin{align}
J_{\theta,S_k(0)} &= \frac{1}{\sigma^2} \sum_{t=0}^T \sum_{i=1}^2 \frac{\partial \mu_i(t)}{\partial \theta} \frac{\partial \mu_i(t)}{\partial S_k(0)} \\
J_{\theta,I_k(0)} &= \frac{1}{\sigma^2} \sum_{t=0}^T \sum_{i=1}^2 \frac{\partial \mu_i(t)}{\partial \theta} \frac{\partial \mu_i(t)}{\partial I_k(0)}
\end{align}
...
where the summations are over $t \in \{0,1,\ldots,T\}$ and $i \in \{1,2\}$.
