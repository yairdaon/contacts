...
Supporting Information). We integrate data from CDC FluView
...
from absolute humidity data~\citep{daon2025, shaman2009} by finding a
phase shift $\varphi$ that maximizes the correlation between negative
absoulute humidity values and a sine whose phase is shifted by
$\varphi$. We define each flu season as $T = 20$ weekly observations
...
\subsection{Seasonal ARIMA Forecast Benchmark}\label{subsec:sarimabench}
We fit a seasonal ARIMA model to each state's weekly per-capita
pneumonia and influenza mortality, with and without exogenous
covariates drawn from other states. For a target state $A$, the model
...
the training split by maximizing AIC\citep{hyndman2008}.

For each ordered pair of states $(A, B)$ with $B \neq A$, we fit
eq.~\eqref{eq:sarima_model} under four configurations of
$\mathbf{x}_{t-1}$:
\begin{align*}
  &\text{(null)} && \boldsymbol{\beta} = 0; \\
  &\text{(nav)}  && \mathbf{x}_{t-1} = (\bar y_{t-1}); \\
  &\text{(biv)}  && \mathbf{x}_{t-1} = (y_{B, t-1}); \\
  &\text{(both)} && \mathbf{x}_{t-1} = (y_{B, t-1},\; \bar y_{t-1});
\end{align*}
where $\bar y_{t} = (1/47)\sum_{S} y_{S, t}$ is the lag-one national
average across all 47 states. We train on 2010--2019 and test on
...
Two mechanisms can produce a bilateral improvement of (biv) over
(null): \emph{(I)}~directed predictability, in which $B$'s past
actually forecasts $A$'s future beyond $A$'s own past and shared
seasonality; or \emph{(II)}~shared national forcing, in which $B$'s
lagged value is a noisy proxy for the current week's national-level
flu-season severity that $\Phi y_{A,t-52}$ only partially absorbs. The
two mechanisms make opposite predictions for the headline comparison
of (both) to (nav)---i.e.\ for the question of whether adding $B$'s
lag-one mortality further reduces RMSE on top of a SARIMA that already
includes the national average. Under \emph{(I)}, $y_{B,t-1}$ carries
directional information that $\bar y_{t-1}$ cannot, so adding it on
top of the national average should still reduce RMSE. Under
\emph{(II)}, $\bar y_{t-1}$ is a much cleaner estimator of the shared
severity signal that $y_{B,t-1}$ proxies only noisily, so adding the
neighbour state on top of the national average should be redundant.
