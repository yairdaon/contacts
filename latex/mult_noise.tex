...
Let:
\begin{itemize}
\item $\mu_i(t) = \beta(t) S_i(t) \sum_j C_{ij} I_j(t)$ is the mean
  incidence for region $i$ at time $t$
\item $\varepsilon_i(t) \sim \mathcal{N}(0, \sigma^2)$ are independent
  noise terms
\item $\sigma$ represents the \emph{relative} standard deviation
  (e.g., $\sigma = 0.05$ for 5\% noise)
\end{itemize}
We take the following observation model:
\begin{equation}
Y_i(t) = \mu_i(t) \cdot (1 + \varepsilon_i(t))
\end{equation}
Thus our observation vector is $\mathbf{Y} = [Y_1(0), Y_2(0),
  \ldots, Y_1(T-1), Y_2(T-1)]^T$. We can verify:
