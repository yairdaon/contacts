...
Let: $\mu_i(t) = \beta(t) S_i(t) \sum_j C_{ij} I_j(t)$ the mean
incidence for region $i$ at time $t$, and $\varepsilon_i(t) \sim
\mathcal{N}(0, \sigma^2)$ are independent noise terms. The parameters
we try to infer are $\mathbf{\phi} = (\theta, S_1(0), I_1(0), S_2(0),
I_2(0))$ --- we assume $\sigma^2$ is known.
...
\subsection{Cramér-Rao Lower Bound Approach}
...
The \emph{Cramér-Rao Lower Bound (CRLB)} provides such fundamental
limits by establishing that for any unbiased estimator:
\begin{equation}
\text{Cov}(\hat{\boldsymbol{\phi}}) \succeq \mathbf{J}^{-1}
\end{equation}
where $\mathbf{J}$ is the \emph{Fisher Information Matrix} and $\mathbf{A} \succeq \mathbf{B}$ means $\mathbf{A} - \mathbf{B}$ is positive semidefinite.

For observations $\mathbf{Y}$ with likelihood
$f(\mathbf{Y}|\boldsymbol{\phi})$, the Fisher Information Matrix can
be computed as:

\begin{equation}
[\mathbf{J}]_{kl} = -\mathbb{E}\left[\frac{\partial^2 \log
    f(\mathbf{Y}|\boldsymbol{\phi})}{\partial \phi_k \partial
    \phi_l}\right] = \mathbb{E}\left[\frac{\partial \log
    f(\mathbf{Y}|\boldsymbol{\phi})}{\partial \phi_k}\frac{\partial
    \log f(\mathbf{Y}|\boldsymbol{\phi})}{\partial \phi_l}\right]
\end{equation}

We will employ the latter approach since it seemed easier. For the
connectivity parameter $\theta$, the Cramér-Rao lower bound on its
variance is found numerically using the Cholesky factorization
$\mathbf{J} = LL^t$:
\begin{equation*}
