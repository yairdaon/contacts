...
therefore routinely encode inter-regional connectivity as an input
derived from these external sources rather than from case data
themselves. A natural modelling question sits beneath this practice:
does information from other regions actually help forecast a given
region's flu incidence, and if so, what is the source of the
improvement?
...
models, and graph or neural-network
architectures~\citep{pei2018, osthus2021, lu2019, aiken2021, deng2020}.
Standard time-series methods similarly suggest US-state ILI forecasts
benefit from including other states as covariates~\citep{thivierge2025},
with a modest but consistent improvement in skill. (For a contrary
result in the European setting, see~\citep{kramer2020}.) We reproduce
this picture for our 47 US states: adding one state's lag-one mortality
as a covariate reduces test-period RMSE for 65\% of (target, covariate)
pairs (Fig.~\ref{fig:sarima_heatmap}; see Methods).

\textbf{We challenge this claim in three steps.} First, we show via
simulation that the connectivity parameter is fundamentally
unidentifiable when epidemics are entrained by common seasonal forcing
(\nameref{subsec:crlb_inflated}). Second, we apply this analysis to the
real US data and find that the surveillance record contains no
statistical evidence of inter-regional coupling for any state pair
(\nameref{subsec:us_real_data}). Third, we return to the forecasting
...
\subsection*{Entrainment Inflates the CRLB}\label{subsec:crlb_inflated}
If the apparent state-covariate signal of
Fig.~\ref{fig:sarima_heatmap} reflects directional inter-regional
coupling, the connectivity parameter $\theta$ must in principle be
inferable from epidemiological data. We test this by computing the
statistical power of a CRLB-based one-sided test for $\theta > 0$ in
a two-region SIR model under in-phase and out-of-phase forcing (see
Methods). Figure~\ref{fig:power} shows this power across a grid of
connectivity values $\theta$ and seasonal forcing amplitudes
$\delta$, with initial conditions drawn from the empirical
distribution of real-data fits (SI~Fig.~\ref{fig:si_ics}). Under
in-phase forcing, power remains at the nominal $\alpha = 0.05$ level
across the entire grid: no test can detect inter-regional coupling
regardless of its true magnitude. Under out-of-phase forcing, power
recovers and exceeds $0.5$ over much of the parameter space. The red
star marks the flu operating point ($\theta = 0.05$, $\delta = 0.517$;
see Methods), which falls deep in the un-detectable region.
Entrainment therefore makes inter-regional coupling fundamentally
unidentifiable from seasonal influenza surveillance data. A
complementary mechanistic view, showing how much the CRLB itself is
inflated by in-phase forcing, is provided in
SI~Fig.~\ref{fig:si_crlb_ratio}.

\begin{figure}[t!]
\centering
\includegraphics[width=\linewidth]{../pix/power.pdf}
\caption{Statistical power of a CRLB-based one-sided test for
  $\theta > 0$ at $\alpha = 0.05$, computed under the 12-season
  aggregation used in the real-data analysis
  ($\mathrm{CRLB}_{12} = \mathrm{CRLB}_1 / 12$). \textbf{Top:}
  in-phase forcing ($\varphi_1 = \varphi_2$). \textbf{Bottom:}
  out-of-phase forcing ($\varphi_1 - \varphi_2 = \pi$). Initial
  conditions are drawn from the empirical distribution of per-season,
  per-state values (SI~Fig.~\ref{fig:si_ics}). Red star: flu
  operating point ($\theta = 0.05$, $\delta = 0.517$).}
\label{fig:power}
\end{figure}

\subsection*{Inter-Regional Connectivity in the US via Influenza}\label{subsec:us_real_data}
The simulation analysis predicts that for US influenza, where states
share strong seasonal forcing and similar phases, the surveillance
record should carry essentially no information about inter-regional
coupling. We test this prediction directly. We analyze excess
pneumonia and influenza mortality from 47 US states over 12 flu
seasons. Figure~\ref{fig:seasons} shows the raw weekly death counts
...
$\hat\theta$ themselves spread roughly uniformly over $[0, 0.5]$. The
apparent forecast benefit of Fig.~\ref{fig:sarima_heatmap} therefore
cannot be the signature of directional inter-regional coupling: no
estimator can recover such coupling from this surveillance record.
...
another state's lag-one mortality is not directional inter-regional
coupling, what is it? A natural alternative is that $y_{B,t-1}$ acts
as a noisy proxy for the current week's national-level severity. We
test this by adding the national-average mortality $\bar y_{t-1}$ as
an additional covariate. On the (target, covariate) pairs for which
all four SARIMA configurations (Methods) converged, the lag-one
...
This is exactly the prediction of mechanism (ii) (Methods): the
modest effect visible in
...
The data are therefore consistent with a common-cause graphical
structure $Y_B \leftarrow \bar Y \to Y_A$, in which national-level
severity confounds the apparent state-to-state Granger causality,
rather than a directed-coupling structure $Y_B \to Y_A$.
...
For endemic seasonal pathogens such as influenza~\citep{viboud2006,
  tamerius2013}, our results therefore imply a single operational
...
less coherent seasonal forcing produces out-of-phase outbreak
...
%
%
where $t$ is measured in years. Then, following~\citep{edlund2011,
...
$\delta \in [0, 1)$, comparing in-phase ($\varphi_1 = \varphi_2$) and
  out-of-phase ($\varphi_1 - \varphi_2 = \pi$) epidemics. At each grid
  ...
  each configuration 500 times
  with fresh IC draws. We then compute the statistical power of a
  one-sided $\alpha = 0.05$ Wald test for $\theta > 0$ under the
  asymptotic Normal approximation of the MLE:
...
aggregated CRLB of eq.~\eqref{eq:crlb_multi} (exact when per-season
...
y_{t-1}$ signal. Equivalently, it is a \emph{conditional} Granger
causality test~\citep{geweke1984}: the bivariate
comparison (state vs.\ univariate) probes whether $Y_B$
Granger-causes $Y_A$ unconditionally, while the conditional comparison
(both vs.\ national-average) probes Granger causality from $Y_B$ to
$Y_A$ given $\bar Y$. A positive bivariate result paired with a null
conditional result is diagnostic of a common-cause graphical
structure $Y_B \leftarrow \bar Y \to Y_A$~\citep{pearl2009}, in which
the apparent $Y_B \to Y_A$ relationship is confounded by shared
...
\dataavail{Pneumonia and influenza mortality data are publicly
  ...
  (2017--2025); these are de-identified, population-level surveillance
  data and the study is exempt from IRB review. Absolute humidity data
  underlying phase estimation are from~\citep{shaman2009}. Analysis
  code (Python; sensitivity equations, CRLB computation, multi-start
  MLE, SARIMA fitting, figure generation) and processed data are
  available from the corresponding author upon request and will be
  deposited in a public repository (Zenodo) upon acceptance.}
