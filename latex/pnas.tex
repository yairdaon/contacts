...
\significancestatement{US public-health agencies forecast seasonal
  ...
  show that when influenza epidemics rise and fall together across
  states, the connection strength is impossible to learn from the
  data: no estimator, however sophisticated, can distinguish a
  connected system from an unconnected one. An independent
  forecast-skill analysis confirms that lagged mortality from
  neighboring states carries no information beyond a national-severity
  signal. We conclude that inter-regional coupling should be dropped
  from operational influenza forecasts, simplifying preparedness
...
other mechanism is unclear. We set up a mechanistic SIR model for
seasonal influenza and estimate its ability to identify regional
connectivity under seasonal forcing via the Cram\'er-Rao Lower
Bound. We find that connectivity is far from identifiable even in this
idealized settings. Next, we fit the SIR model to 12 seasons of excess
pneumonia and influenza mortality across 47 US states. We arrive at
similar results: confidence-intervals around the estimated
connectivity overlap zero for the vast majority of state
pairs. Independently, employing a standard time-series forecast model,
we show that another state's pneumonia and influenza mortality
provides no improvement in forecast skill beyond the national average
rate of pneumonia and influenza mortality. Thus, conditional Granger
causality suggests that connectivity is not a causal driver of
influenza. Connectivity is therefore unsupported by either analysis
and should be dropped from forecast models used for influenza
...
only a unidirectional forcing, while the latter requires bidirectional
coupling --- population mixing, in the case of influenza. In the
\Endparasplit \noindent United States, all states share similar
intrinsic transmission dynamics and are driven by similar annual
environmental forcing. Thus, state-level mortality levels can be
strongly correlated without any inter-regional coupling.
...
connectivity is not relevant, or (ii) connectivity is compensated for
by epidemic initial conditions. Since forecast models typically fit
the initial conditions of an epidemic, our analysis suggests that
connectivity may, in fact, be rendered redundant. We proceed to test
...
evidence of inter-regional coupling.

Second, in an independent line of analysis, we return to the
forecasting framework of Fig.~\ref{fig:sarima_heatmap} and show that
...
support. An unidentifiable parameter is unlikely to improve forecast
skill and should be removed from forecast models in accordance with
Occam's razor.
...
directional inter-regional coupling. The real-data uncertainties
...
$A$'s future beyond the $A$'s own past and shared seasonality; or (ii)
shared national forcing, in which the $B$'s lagged value is a noisy
...
$y_{A,t}$, so by Reichenbach's common cause priniciple and temporal
...
than coupled by inter-regional mixing.
...
of another state adds essentially nothing beyond the lag-one
...
directional inter-regional coupling.
...
covariate alone produces a median RMSE reduction of $7.4\%$ and
...
than inter-regional connectivity, and our Cram\'er-Rao analysis gives
