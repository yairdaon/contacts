...
surface. We generate synthetic data from the SIR model with true
connectivity $\theta = 0.05$ and evaluate the log-likelihood at the
true initial conditions across a grid of $\theta$ values, sweeping
over four phase differences.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../pix/likelihood_surface.pdf}
\caption{Log-likelihood as a function of $\theta$ for four phase
  offsets between regions. Left: similar initial conditions
  ($S_i(0)/N_i \in [0.7, 0.95]$). Right: different initial conditions
  ($S_i(0)/N_i \in [0.5, 0.95]$). Dashed line: true $\theta = 0.05$.
  With similar ICs (left), synchronized epidemics produce a flat
  likelihood; increasing phase difference sharpens the peak. With
  different ICs (right), even synchronized epidemics show a clear
  peak.}
\label{fig:likelihood}
\end{figure}

With similar initial conditions (left panel), the synchronized curve
($\varphi_1 - \varphi_2 = 0$) is nearly flat: the likelihood barely
changes across the parameter space. An optimizer would converge to
some $\hat\theta$, and the fit might look reasonable, but the
parameter is uninformed by the data. This is the regime where an
analyst could obtain a misleading estimate without realizing it.
Increasing the phase difference progressively sharpens a peak near
the true value. With different initial conditions (right panel), even
